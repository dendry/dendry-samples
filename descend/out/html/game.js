(function() {
  ...
  var findOnePerLevelScenes = function() {
    ...
    onePerGameSceneIds = game.tagLookup['one-per-game'];
    if (onePerGameSceneIds === undefined) onePerGameSceneIds = {};
  };
  ...
  var updateLevel = function() {
    ...
    // todo remove one per game scenes
    for (var i = 2; i < 9; ++i) {
      ...
      if (onePerGameSceneIds[thisSceneId]) {
        validScenes.splice(thisSceneIndex, 1);
      }
  ...
  var updateLocation = function() {
    ...
    // Move out any prior room qualities
    if (currentLocation !== undefined) {
      var prevRoomQualities = roomQualities[currentLocation-1];
      for (var quality in engine.state.qualities) {
        if (quality.substr(0, 5) === 'room-') {
          var val = engine.state.qualities[quality];
          if (val !== undefined) {
            prevRoomQualities[quality] = val;
            engine.state.qualities[quality] = undefined;
          }
        }
      }
    }
    ...
    // Move in any room qualities we previously had
    var thisRoomQualities = roomQualities[currentLocation-1];
    for (var quality in thisRoomQualities) {
      if (thisRoomQualities[quality] !== undefined) {
        engine.state.qualities[quality] = thisRoomQualities[quality];
      }
    }
  ...
  var handleSignal = function(signal) {
    ...
    case 'stats': if (signal.event === 'quality-change') {
      console.log(signal);
      switch(signal.id) {
        case 'pc-level': $("#pc-level").html(signal.now); break;
        case 'hits': $("#hits").html(signal.now); break;
        case 'max-hits': $("#max-hits").html(signal.now); break;
        case 'xp':
          $("#xp").html(signal.now+"/"+engine.state.qualities.level);
          break;
      }}
      break;
